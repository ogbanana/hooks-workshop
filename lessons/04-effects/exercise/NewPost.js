...
export default function NewPost({ takeFocus, date, onSuccess, showAvatar }) {
  ...
  /* This here is the state changer */
  const [message, setMessage] = useState(
    getLocalStorageValue(newKey) || 'Ran around the lake.')
  ...
  useEffect(() => {
    setLocalStorage(newKey, message)},[newKey, message]
    )
    
  /* Getting access to the real DOM element */  
  const inputRef = useRef()
  /* The effect attached to the ref */
  useEffect(()=>{
    takeFocus && inputRef.current.focus()}, [takeFocus,inputRef]) 
